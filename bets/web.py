...
CSS = """
  ...
  * { box-sizing: border-box; }
  ...
  .scoreboard-spark-path {
    ...
    /* Animated draw-in: pathLength="1" attribute on the element normalizes
       the path to 1 unit; dasharray/offset both 1 starts it invisible.
       The keyframe sweeps offset to 0 over 1.1s. CSS-only — no JS timing. */
    stroke-dasharray: 1;
    stroke-dashoffset: 1;
    animation: scoreboard-spark-draw 1.1s cubic-bezier(0.22, 1, 0.36, 1) forwards;
  }
  ...
  @keyframes scoreboard-spark-draw {
    from { stroke-dashoffset: 1; }
    to   { stroke-dashoffset: 0; }
  }
  .scoreboard-spark-tip {
    /* Last-point marker — fades in just after the line finishes drawing. */
    r: 2.5;
    opacity: 0;
    animation: scoreboard-spark-tip-fade 0.4s ease 0.95s forwards;
  }
  .scoreboard-spark-tip.pos  { fill: var(--green); }
  .scoreboard-spark-tip.neg  { fill: var(--red); }
  .scoreboard-spark-tip.flat { fill: var(--muted); }
  ...
  /* "Browse all pitchers" twisty around the full slate table. Default-
     collapsed; open state persisted in localStorage by wireSlateTableToggle. */
  details.slate-table-wrap {
    margin: 16px 0 0;
    border: 1px solid var(--border);
    border-radius: 8px;
    background: var(--panel);
  }
  details.slate-table-wrap > summary {
    cursor: pointer;
    user-select: none;
    list-style: none;
    padding: 10px 14px;
  ...
  details.slate-table-wrap > summary::-webkit-details-marker { display: none; }
  details.slate-table-wrap > summary::marker { content: ""; }
  details.slate-table-wrap > summary::after {
    content: "▸";
    color: var(--muted);
    font-size: 11px;
    transition: transform 0.15s ease;
  }
  details.slate-table-wrap[open] > summary::after { transform: rotate(90deg); }
...
(() => {{
    ...
    document.querySelectorAll(".pick-card[data-pitcher-id]").forEach(card => {{
      ...
      if (html) {{
        if (existing) {{
          ...
          // No existing row — inject after the matchup line so badges
          // land in the same slot as on initial render.
          const matchup = card.querySelector(".pick-card-matchup");
  ...
  function pitcherTabHTML(heroSection, parlaySection, slateBody, resultsSection, cnt) {{
    ...
    <details class="slate-table-wrap" id="slate-table-twisty">
  ...
  function renderScoreboardSparkline(cum, sign) {{
    ...
    return `<svg class="scoreboard-spark" viewBox="0 0 ${{w}} ${{h}}" preserveAspectRatio="none">
      <path class="scoreboard-spark-area ${{sign}}" d="${{areaD}}" />
      <path class="scoreboard-spark-path ${{sign}}" d="${{pathD}}" pathLength="1" vector-effect="non-scaling-stroke" />
      <circle class="scoreboard-spark-tip ${{sign}}" cx="${{lastX}}" cy="${{lastY}}" />
    </svg>`;
  }}
  ...
  function renderScoreboardModelCol(track) {{
    ...
    // Cumulative units, one point per dated day in the picks set.
    const byDate = {{}};
    for (const p of picks) {{
      byDate[p.date] = (byDate[p.date] || 0) + p.pnl;
    }}
  ...
  function renderScoreboardBankrollCol(betsState) {{
    ...
    // empty days, so the curve reads like an equity curve).
    const dailyPnl = computeDailyBetsPnl(recentSettled);
    const byDate = {{}};
    for (const u of dailyPnl) byDate[u.date] = u.pnl;
    const cum = [];
    let running = 0;
    for (let i = 29; i >= 0; i--) {{
      const d = dateInChicago(-i);
