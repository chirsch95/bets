...
CSS = """
  ...
  * { box-sizing: border-box; }
  ...
  /* An inactive tab panel must stay hidden even on is-bets hosts, where
     `html.is-bets .bets-only { display: revert }` (higher specificity) would
     otherwise force the bets-panel visible on every tab — leaking its
     "Loading…" placeholder onto e.g. the UD Lab screen. */
  ...
  .udlab-upload {
    ...
    padding: 7px 14px; border-radius: 8px; font-weight: 600; font-size: 0.95em;
  }
  .udlab-upload:hover { border-color: var(--muted); }
  .udlab-record-btn { background: var(--panel); color: var(--text); border: 1px solid var(--border);
    padding: 7px 14px; border-radius: 8px; font: inherit; font-weight: 600; cursor: pointer; }
  .udlab-record-btn:hover { border-color: var(--muted); }
  /* "My record" panel — betting results + edge-band tuning report. */
  .udlab-record { margin: 0 0 12px; padding: 12px 14px; font-size: 0.9em; line-height: 1.5;
    border: 1px solid var(--border); border-left: 3px solid var(--brand-blue);
    border-radius: 8px; background: var(--panel); color: var(--text); }
  .udlab-record h4 { margin: 0 0 8px; font-size: 1em; }
  .udlab-record .rec-line { margin: 3px 0; }
  .udlab-record .rec-note { color: var(--muted); margin-top: 6px; }
  .udlab-rec-table { width: 100%; border-collapse: collapse; margin-top: 8px; }
  .udlab-rec-table th, .udlab-rec-table td { padding: 4px 8px; border-bottom: 1px solid var(--border); text-align: right; }
  .udlab-rec-table th:first-child, .udlab-rec-table td:first-child { text-align: left; }
  .udlab-rec-table .above { color: var(--green); font-weight: 600; }
  .udlab-rec-table .below { color: var(--muted); }
  /* Rows just filled from a screenshot — review before Save. */
  tr.udlab-imported > td { box-shadow: inset 3px 0 0 var(--brand-blue); }
  ...
  .udlab-mult-input {
    width: 5.5em; padding: 5px 6px; text-align: right;
    border: 1px solid var(--border); border-radius: 6px;
    background: var(--bg); color: var(--text); font-size: 1em;
  }
  .udlab-mult-input + .udlab-mult-input { margin-left: 4px; }
  .udlab-mult-input:focus { outline: 2px solid var(--green); outline-offset: 1px; }
  /* A multiplier the user moved off the default 1.00 (UD has priced it). */
  .udlab-mult-input.udlab-priced {
    border-color: var(--yellow); background: var(--yellow-bg); font-weight: 600;
  }
  /* Reliever/opener rows — sub-floor line, model over-projects. De-emphasized. */
  tr.udlab-reliever { opacity: 0.5; }
  .udlab-rp {
  ...
  /* Lineup-posted border on UD parlay cards (Chad's request): red while any
     leg's opp lineup is TBD, green once all are confirmed. */
  ...
  /* △ unpriced-leg caution chip (replaced the ⏰ place-early badge
     2026-06-04): legs UD hasn't priced rest on the model alone — early
     tracking has them underperforming priced legs, so amber, not green. */
  .udlab-caution {
    font-size: 0.82em; font-weight: 700; color: #1a1400; background: var(--yellow);
  ...
  .udlab-pl-card.boost-target { border-width: 2px; border-color: #d4a017; }
  /* FUN bucket: free-credit / fun-budget longshots (three-bucket policy). */
  .udlab-fun-section { margin-top: 16px; padding-top: 10px; border-top: 1px dashed var(--border); }
  .udlab-pl-card.udlab-fun-card { border-style: dashed; }
  .udlab-boost-target { font-weight: 700; color: #d4a017; }
  .udlab-boost-banner { border: 1px solid var(--border); border-left: 3px solid #d4a017;
    border-radius: 8px; padding: 8px 10px; margin: 8px 0; font-size: 0.92em; }
  /* "Today's one bet" hero — the boost banner promoted to the daily
     decision (one-bet-per-day policy, 2026-07-02). */
  .udlab-boost-banner.udlab-hero { border-width: 2px; border-left-width: 4px;
    border-color: #d4a017; padding: 10px 14px; }
  .udlab-boost-banner.udlab-hero.tappable { cursor: pointer; }
  .udlab-boost-banner.udlab-hero.tappable:hover { background: rgba(212, 160, 23, 0.07); }
  .udlab-hero-title { font-weight: 700; font-size: 1.05em; color: #d4a017; margin-bottom: 4px; }
  .udlab-hero-card { font-weight: 600; font-size: 1.02em; margin-bottom: 2px; }
  .udlab-hero-stats { font-size: 0.95em; margin-bottom: 4px; }
  .udlab-hero-stats .pos { color: var(--green); }
  .udlab-hero-note { font-size: 0.88em; color: var(--muted); }
...
(() => {{
  ...
  // Chad's recurring UD promo: a 30% payout boost, usually one per day.
  // Applied multiplicatively to the whole entry payout (ledger-confirmed:
  // boosted 2-leg entries cluster at ~1.3× the unboosted multiplier).
  // NOT the same thing as UD_PAYOUTS_BOOSTED above — that's UD's own
  // higher base for multiplier-priced entries; this is the promo on top.
  const UD_BOOST = 1.3;
  // One-bet-per-day policy (2026-07-02 restart review): the daily play is
  // the boost-target card at this stake, boosted, or no bet at all.
  // Unboosted standard-payout cash graded ≈ −7% lifetime; the boost is the
  // profit source. Prefilled into the Bets form on card tap.
  const BOOST_STAKE = 5;
  ...
  function evaluateParlay(legs) {{
    ...
    // What the entry actually pays: UD's flat power play for this leg count
    // (the sportsbook decimal product is unattainable — UD is the only book
    // in play since 2026-05-25). evUD/evUDBoost are the money numbers;
    // book-odds ev is kept for reference/auditing only (2026-06-09).
    const udMult = UD_PAYOUTS[legs.length] || null;
    return {{
      legs,
      ...
      // EV per $1 at book odds (legacy reference, not bettable).
      ev: hit * (dec - 1) - (1 - hit),
      udPayout: udMult,
      evUD: udMult === null ? null : hit * udMult - 1,
      evUDBoost: udMult === null ? null : hit * udMult * UD_BOOST - 1,
    }};
  }}
  ...
  function udCompare(r, udLine, hi, lo) {{
    ...
    // UD's own implied P(over) from its multipliers, if priced. 1/1 (the
    // prefill default) counts as symmetric → fall back to the sharp line.
    const priced = multPriced(hi, lo);
  ...
  function renderParlayCard(p) {{
    ...
    // Color and EV figures follow the UD-payout math (what the ticket
    // actually pays), not the legacy book-odds ev (2026-06-09).
    const evUD = p.evUD === null ? p.ev : p.evUD;
    const evCls = evUD > 0.02 ? "pos" : evUD < -0.02 ? "neg" : "flat";
    const payStr = p.udPayout ? p.udPayout + "×" : "—";
    const evStr = (evUD >= 0 ? "+" : "") + evUD.toFixed(2);
    const evBoostCls = p.evUDBoost === null ? "flat" : (p.evUDBoost > 0.02 ? "pos" : p.evUDBoost < -0.02 ? "neg" : "flat");
    const evBoostStr = p.evUDBoost === null ? "—" : (p.evUDBoost >= 0 ? "+" : "") + p.evUDBoost.toFixed(2);
    ...
    // Tap-time card snapshot (2026-06-04, mirrors udParlayCard): the card's
    // displayed numbers ride the handoff into the saved bet (suggested_card)
    // so the provenance + claimed EV survive suggester churn.
    const r4 = v => v == null ? null : Math.round(v * 1e4) / 1e4;
    const cardMeta = {{
      source: "pitchers",
      // ev = UD-payout EV (the honest claim) as of 2026-06-09; ev_book keeps
      // the legacy book-odds figure so old/new snapshots stay comparable.
      ev: r4(p.evUD !== null ? p.evUD : p.ev), ev_boost: r4(p.evUDBoost),
      ev_book: r4(p.ev), ud_payout: p.udPayout,
      prob: r4(p.combinedHit), edge: r4(p.combinedEdge),
    ...
    return `<div class="parlay-card ${{evCls}} ${{lineupCls}}"${{dataAttr}} title="${{escapeHTML(tip)}}">
      ...
      <div class="parlay-stats">
        <div class="parlay-stat"><span class="parlay-stat-label" title="Underdog flat power-play payout for this leg count">Payout</span><span class="parlay-stat-val">${{payStr}}</span></div>
        <div class="parlay-stat"><span class="parlay-stat-label" title="Calibrated probability every leg hits">Hit %</span><span class="parlay-stat-val">${{(p.combinedHit * 100).toFixed(1)}}%</span></div>
        <div class="parlay-stat"><span class="parlay-stat-label" title="EV per $1 at the UD payout, no boost">EV / $1</span><span class="parlay-stat-val ${{evCls}}">${{evStr}}</span></div>
        <div class="parlay-stat"><span class="parlay-stat-label" title="EV per $1 with the 30% promo boost applied">🚀 30%</span><span class="parlay-stat-val ${{evBoostCls}}">${{evBoostStr}}</span></div>
      </div>
    </div>`;
  }}
  ...
  function renderParlaySuggestions(rows) {{
    ...
    const legs = focus
      ...
      // model-market disagreements — at UD's fixed payout, win probability
      // is the ranking, and sorting by |edge| pre-loaded the pool with
      // exactly the phantom-edge legs (2026-06-09; keep in sync with
      // parlay_suggest.py).
    ...
    const buildSection = (k, top, label, excludePids) => {{
      ...
      const sorted = combos(legs, k)
        ...
        .map(evaluateParlay)
        // Keep any card playable with the 30% boost; cards that are only
        // +EV boosted get labeled on the card itself. Rank by UD-payout EV
        // (= win probability, since the payout is flat per leg count).
        .filter(p => p.evUDBoost !== null && p.evUDBoost > 0)
        .sort((a, b) => b.evUD - a.evUD);
    ...
    // 2-LEG ONLY (2026-07-02 restart policy): paid 3-legs graded −49.9%
    // lifetime, so the 3-leg section was retired from both suggestion
    // surfaces (here and the UD Lab). parlay_suggest.py still writes
    // three_leg snapshot rows so the retired section keeps grading as a
    // shadow (same precedent as the 0.065 shadow floor). The ⚠ overlap
    // badge left with it.
    ...
    const parlayNote = DATA_ONLY
      ...
      : "2-leg only (3-leg cash retired 2026-07-02, −49.9% lifetime) · ranked by EV at UD payouts (3×) with calibrated probabilities · one leg per game · games within 3 hrs · capped per pitcher · 🚀 = with the 30% boost (cards kept if +EV boosted) · verify prices on the UD Lab before betting";
  ...
  let _udImported = new Set(); // pids just filled from a screenshot — review state
  ...
  // UD Higher/Lower payout multipliers for a row. Default to 1.00 (a
  // symmetric pick) so the inputs prefill with 1 and Chad can increment /
  // decrement from there. udCompare treats 1/1 as symmetric (no UD pricing).
  function udMults(r) {{
    const e = udEntry(r);
    return {{ hi: e && e.hi != null ? e.hi : 1, lo: e && e.lo != null ? e.lo : 1 }};
  }}
  // Has the user moved either multiplier off the default 1.00? (UD priced it.)
  function multPriced(hi, lo) {{
    return !(Math.abs((hi == null ? 1 : hi) - 1) < 1e-9 && Math.abs((lo == null ? 1 : lo) - 1) < 1e-9);
  }}
  ...
  function udLabRow(r) {{
    ...
    // Reliever / opener gate: a line below the focus floor means the book is
    // treating this arm as <2 innings, but our model projects a full start —
    // a phantom edge. Mirror the live MIN_LINE_FOR_FOCUS gate so these never
    // become a UD verdict or parlay leg.
    const reliever = udLine !== null && udLine < MIN_LINE_FOR_FOCUS;
    const ud = reliever ? {{ cls: "noise", label: "RP — skip" }} : udVerdict(c);
    ...
    // into repaintGameTimeCells(): countdown ticks every minute, live K /
    // Final states paint in from the shared 60s poll, and row-locked dims
    // the row once first pitch passes. Abbreviated opp name — the Lab table
    // is already dense.
    const iso = r.game_datetime_utc || "";
    const pidNum = parseInt(pid, 10);
    const liveK = isNaN(pidNum) ? null : _liveByPid.get(pidNum);
    const tcell = renderGameTimeCell(iso, liveK);
    const isoAttr = iso ? ` data-game-iso="${{escapeHTML(iso)}}"` : "";
    const pidAttr = !isNaN(pidNum) ? ` data-pitcher-id="${{pidNum}}"` : "";
    return `<tr class="${{changed ? "udlab-changed" : ""}}${{reliever ? " udlab-reliever" : ""}}${{_udImported.has(pid) ? " udlab-imported" : ""}}${{tcell.locked ? " row-locked" : ""}}">
      ...
      <td class="udlab-mult-cell">
        <input class="udlab-mult-input${{Math.abs(m.hi - 1) > 1e-9 ? " udlab-priced" : ""}}" type="number" inputmode="decimal" step="0.01" min="0.1" max="50"
          data-pid="${{pid}}" data-field="hi" value="${{m.hi}}" aria-label="Higher multiplier for ${{escapeHTML(r.pitcher || "")}}">
        <input class="udlab-mult-input${{Math.abs(m.lo - 1) > 1e-9 ? " udlab-priced" : ""}}" type="number" inputmode="decimal" step="0.01" min="0.1" max="50"
          data-pid="${{pid}}" data-field="lo" value="${{m.lo}}" aria-label="Lower multiplier for ${{escapeHTML(r.pitcher || "")}}">
      </td>
  ...
  function udBuildLegs(rows) {{
    ...
    for (const r of rows) {{
      ...
      // Reliever/opener gate — model over-projects sub-floor lines as full starts.
  ...
  // All positive-EV k-leg combos (one leg per game), ranked by EV. NOT
  // deduped or sliced — udSelectParlays does the cross-section selection.
  function udRankedCombos(legs, k) {{
    ...
    return combos(legs, k)
      ...
      .map(combo => {{
        ...
        // evBoost = the same entry with Chad's 30% promo applied. The promo
        // scales the payout, so card *ranking* is identical either way —
  ...
  function udSelectParlays(legs) {{
    const pick = (k, cap) => {{
      ...
      for (const x of udRankedCombos(legs, k)) {{
        ...
        if (x.legs.some(l => used.has(l.pitcher_id))) continue;
        x.legs.forEach(l => used.add(l.pitcher_id));
        out.push(x);
      }}
      return out;
    }};
    ...
    // Boost allocation: the daily 30% boost belongs on the single 2-leg
    // card with the highest boosted EV. 2-LEG ONLY (2026-07-02 restart
    // policy): paid 3-legs graded −49.9% lifetime, so money suggestions —
    // and therefore the boost — never go to a 3-leg card. 3-leg combos are
    // still computed for the measurement journal and the FUN (free-credit)
    // lane, but they can't be today's bet.
    if (two.length) {{
      const best = two.reduce((a, b) => (b.evBoost > a.evBoost ? b : a));
      best.boostTarget = true;
    }}
    return {{ two, three }};
  }}

  // ---- FUN bucket: free-credit / fun-budget longshots (three-bucket policy
  // 2026-06-15). A separate, looser pool that DROPS the 0.08 floor and the
  // 0.15 phantom cap — any leg the model favors (edge > 0) qualifies,
  // including the big-payout phantom picks the money pools exclude. Ranked by
  // expected payout (p × payout), NOT edge, to maximize the "watch it hit"
  // upside on zero-risk (free credits) or accepted-−EV (fun budget) money.
  // Reliever gate still applies. Mirror: ud_parlay.build_fun_legs /
  // ud_fun_parlays.
  function udBuildFunLegs(rows) {{
    const legs = [];
    for (const r of rows) {{
      const pid = String(r.pitcher_id || "").trim();
      if (!pid) continue;
      const udLine = effUDLine(r);
      if (udLine === null || udLine < MIN_LINE_FOR_FOCUS) continue;
      const m = udMults(r);
      const c = udCompare(r, udLine, m.hi, m.lo);
      if (!c || c.pickProb === null || !c.dir || c.edge === null || c.edge <= 0) continue;
      const gpk = parseInt(r.game_pk, 10);
      legs.push({{
        pitcher: r.pitcher || "", pitcher_id: pid, dir: c.dir,
        prob: c.pickProb, udLine: c.udLine, mult: c.sideMult || 1,
        edge: c.edge, priced: c.priced, band: "fun",
        lineupPending: lineupPending(r),
        game_pk: isNaN(gpk) ? null : gpk,
        gameTimeISO: r.game_datetime_utc || "",
      }});
    }}
    return legs.sort((a, b) => (b.prob * (b.mult || 1)) - (a.prob * (a.mult || 1)));
  }}

  function udFunCombos(legs, k) {{
    if (legs.length < k) return [];
    const seenGames = (combo) => {{
      const s = new Set();
      for (const l of combo) {{ if (l.game_pk === null) continue; if (s.has(l.game_pk)) return false; s.add(l.game_pk); }}
      return true;
    }};
    const MAX_GAP_MS = 3 * 3600 * 1000;
    const withinWindow = (combo) => {{
      let lo = Infinity, hi = -Infinity;
      for (const l of combo) {{ if (!l.gameTimeISO) continue; const t = Date.parse(l.gameTimeISO); if (isNaN(t)) continue; if (t < lo) lo = t; if (t > hi) hi = t; }}
      if (lo === Infinity || hi === -Infinity) return true;
      return hi - lo <= MAX_GAP_MS;
    }};
    return combos(legs, k)
      ...
      .map(combo => {{
        ...
        const winP = combo.reduce((a, l) => a * l.prob, 1);
        const payMult = combo.reduce((a, l) => a * (l.mult || 1), eb.base);
        const value = winP * payMult;  // expected $ back per $1 (house-money EV)
        return {{ legs: combo, prob: winP, payMult, confirmed: eb.confirmed,
                 value, ev: value - 1, evBoost: value * UD_BOOST - 1, fun: true }};
      }})
      .sort((a, b) => b.value - a.value);
  }}

  function udSelectFunParlays(legs) {{
    const pick = (k, cap) => {{
      const used = new Set();
      const out = [];
      for (const x of udFunCombos(legs, k)) {{
        if (out.length >= cap) break;
        if (x.legs.some(l => used.has(l.pitcher_id))) continue;
        x.legs.forEach(l => used.add(l.pitcher_id));
        out.push(x);
    ...
    return {{ two: pick(2, 2), three: pick(3, 2) }};
  }}

  function udFunCard(x) {{
    const pendingLegs = x.legs.filter(l => l.lineupPending);
    const lineupCls = pendingLegs.length ? "lineup-blocked" : "lineup-ready";
    const lineupTip = pendingLegs.length
      ? "Waiting on lineups: " + pendingLegs.map(l => l.pitcher).join(", ")
      : "All lineups posted — tap to add this longshot (tag it Free credit or Fun budget)";
    const formLegs = x.legs.map(l => ({{ pitcher: l.pitcher, pitcher_id: l.pitcher_id, line: l.udLine, ou: l.dir === "over" ? "O" : "U" }}));
    const r4 = v => v == null ? null : Math.round(v * 1e4) / 1e4;
    const cardMeta = {{
      source: "ud_lab", kind: "fun",
      ev: r4(x.ev), ev_boost: r4(x.evBoost), prob: r4(x.prob), pay_mult: r4(x.payMult),
      value: r4(x.value), confirmed: !!x.confirmed,
      legs: x.legs.map(l => ({{ pid: l.pitcher_id, dir: l.dir, line: l.udLine, mult: r4(l.mult || 1), priced: !!l.priced, band: "fun" }})),
    }};
    ...
    const legHtml = x.legs.map(l =>
      `<div class="udlab-pl-leg">${{escapeHTML(l.pitcher)}}${{l.lineupPending ? ' <span class="lineup-pending">TBD</span>' : ""}} <strong>${{l.dir.toUpperCase()}} ${{l.udLine}}</strong>${{l.mult && l.mult !== 1 ? " @" + l.mult + "×" : ""}} · ${{pct1(l.prob)}}</div>`
    ).join("");
    return `<div class="udlab-pl-card udlab-fun-card ${{lineupCls}}"${{dataAttr}} title="${{escapeHTML(lineupTip)}}">
      <div class="udlab-pl-head">${{x.confirmed ? "" : "~"}}${{x.payMult.toFixed(2)}}× · win ${{pct1(x.prob)}} · returns <strong>${{x.value.toFixed(2)}}×</strong> per $1</div>
      ${{legHtml}}
    </div>`;
  }}

  function udFunColumn(picked, k) {{
    if (!picked.length) return "";
    return `<div class="udlab-pl-col-title">${{k}}-leg longshot</div>${{picked.map(udFunCard).join("")}}`;
  }}

  // Shared tap-to-bet payload for a money card — used by the card itself
  // AND the "Today's one bet" hero banner, so tapping either hands the
  // exact same legs + tap-time snapshot (suggested_card provenance) to the
  // Bets form. Returns "" while any leg's lineup is pending, which keeps
  // the tap target un-clickable until everything posts.
  function udCardDataAttr(x) {{
    const pendingLegs = x.legs.filter(l => l.lineupPending);
    if (pendingLegs.length) return "";
    const unpricedLegs = x.legs.filter(l => !l.priced);
    // Line = the UD line — that's the number actually on the ticket — so
    // the form's yellow "overridden" border correctly flags any divergence
    // from the sportsbook slate line.
    ...
    // Tap-time card snapshot (2026-06-04): the card's claim — EV, win prob,
    // payout, per-leg multipliers — exactly as displayed when tapped. Rides
    // the handoff into the saved bet (suggested_card) so churn in later
    // renders can never rewrite what the suggester promised at placement.
    const r4 = v => v == null ? null : Math.round(v * 1e4) / 1e4;
    const cardMeta = {{
      source: "ud_lab",
      ev: r4(x.ev), ev_boost: r4(x.evBoost), prob: r4(x.prob), pay_mult: r4(x.payMult),
    ...
    return ` data-legs='${{escapeHTML(JSON.stringify(formLegs))}}' data-card='${{escapeHTML(JSON.stringify(cardMeta))}}'`;
  }}

  function udParlayCard(x) {{
    // △ caution (replaced ⏰ "place early" 2026-06-04). The old badge
    // watched for symmetric UD lines diverging from the sharp consensus —
    // a condition UD essentially never produces (0 hits in 364 captured
    // board rows; UD attaches multipliers 46/48 times its line diverges).
    // The opposite signal proved real: legs UD has NOT priced (no
    // multiplier) rest entirely on model-vs-sportsbook disagreement, and
    // early tracking has them underperforming priced legs (3/14 vs 22/33,
    // tiny n). Flag them as lower-confidence instead.
    const unpricedLegs = x.legs.filter(l => !l.priced);
    const caution = unpricedLegs.length > 0;
    ...
    // Cross-tab handoff + tap-time snapshot — shared builder (see
    // udCardDataAttr above). Empty while any leg's lineup is TBD, so the
    // card stays un-clickable until everything posts.
    const dataAttr = udCardDataAttr(x);
    const legHtml = x.legs.map(l =>
    ...
    const overlapBadge = x.overlapsTop2Leg
      ...
      : "";
    const cautionChip = caution
      ? `<span class="udlab-caution" title="${{unpricedLegs.length}} leg(s) UD hasn't priced (no multiplier) — the edge rests only on the model disagreeing with the sportsbook at UD's default line. Early tracking: unpriced legs underperform UD-priced ones. Lower confidence.">△ ${{unpricedLegs.length}} unpriced</span> `
      : "";
    // Boost-aware verdict line. Three states:
    //   ev > 0           → playable cash, even better boosted
    //   ev <= 0 < evBoost → ONLY playable with the 30% boost or a free credit
    //   (evBoost <= 0 cards never reach here — filtered in udRankedCombos)
    ...
    return `<div class="udlab-pl-card ${{lineupCls}}${{x.boostTarget ? " boost-target" : ""}}"${{dataAttr}} title="${{escapeHTML(lineupTip)}}">
      ${{overlapBadge}}
      <div class="udlab-pl-head">${{boostChip}}${{gateChip}}${{cautionChip}}${{x.confirmed ? "" : "~"}}${{x.payMult.toFixed(2)}}× · win ${{pct1(x.prob)}} · <span class="${{x.ev > 0 ? "pos" : "neg"}}">EV ${{(x.ev >= 0 ? "+" : "")}}${{x.ev.toFixed(3)}}</span> · ${{evBoostStr}}</div>
      ${{legHtml}}
    </div>`;
  }}

  function udParlayColumn(picked, k) {{
    if (!picked.length) return "";
    return `<div class="udlab-pl-col-title">${{k}}-leg</div>${{picked.map(udParlayCard).join("")}}`;
  }}
  ...
  function renderUDParlayCompare(rows) {{
    ...
    const sel = udSelectParlays(legs);  // no pitcher repeats across cards
    // Money suggestions are 2-LEG ONLY (2026-07-02 restart policy — paid
    // 3-legs −49.9% lifetime). sel.three keeps being computed so the
    // Python twin's journal grades it as a shadow, but it isn't rendered
    // here; 3-leg longshots live only in the FUN (free-credit) section.
    const two = udParlayColumn(sel.two, 2);
    ...
    const plBody = two
      ? `<div class="udlab-pl-grid"><div>${{two}}</div></div>`
      : `<p class="muted">No positive-EV UD 2-leg parlays from the lines entered yet.</p>`;
    // ── "Today's one bet" hero ────────────────────────────────────────
    // One-bet-per-day policy (2026-07-02 restart review): the daily play
    // is the boost-target card, $BOOST_STAKE, WITH the 30% boost — or no
    // bet at all. The critical review found unboosted standard-payout cash
    // graded ≈ −7% lifetime while all realized profit came from the boost
    // + free credits, so the banner IS the decision, not a footnote: it
    // names the card, the stake, and the no-bet states (boost already
    // spent / nothing qualifies). Tappable like a card — same handoff.
    const twoCards = sel.two;  // 2-leg only — 3-leg cash retired 2026-07-02
    let boostBanner = "";
    if (twoCards.length) {{
      const best = twoCards.find(x => x.boostTarget) || twoCards[0];
      const legsTxt = best.legs.map(l =>
        `${{l.pitcher}} ${{l.dir.toUpperCase()}} ${{l.udLine}}`).join(" + ");
      const pending = best.legs.filter(l => l.lineupPending);
      const heroAttr = udCardDataAttr(best);
      const tapTxt = pending.length
        ? `⏳ waiting on lineups (${{escapeHTML(pending.map(l => l.pitcher).join(", "))}}) — bet only after they post`
        : `tap to add it ($${{BOOST_STAKE}} · tagged boost)`;
      const cashTxt = best.cashEligible
        ? `boost already spent? it's still +EV plain (+${{(best.ev * 100).toFixed(1)}}%) — or just pass`
        : `<strong>boost already spent? pass</strong> — this card is not playable plain (${{(best.ev * 100).toFixed(1)}}%)`;
      boostBanner = `<div class="udlab-boost-banner udlab-hero${{heroAttr ? " tappable" : ""}}"${{heroAttr}}>
        <div class="udlab-hero-title">🎯 Today's one bet — $${{BOOST_STAKE}} with the 30% boost</div>
        <div class="udlab-hero-card">${{escapeHTML(legsTxt)}}</div>
        <div class="udlab-hero-stats">${{best.confirmed ? "" : "~"}}${{best.payMult.toFixed(2)}}× → <strong>${{(best.payMult * UD_BOOST).toFixed(2)}}× boosted</strong> · win ${{pct1(best.prob)}} · EV <span class="pos">+${{(best.evBoost * 100).toFixed(1)}}%</span> boosted</div>
        <div class="udlab-hero-note">${{cashTxt}} · free credits play this same card · ${{tapTxt}}</div>
      </div>`;
    }} else {{
      boostBanner = `<div class="udlab-boost-banner udlab-hero">
        <div class="udlab-hero-title">🎯 No bet today</div>
        <div class="udlab-hero-note muted">No card clears the bar, even with the 30% boost. Passing costs nothing — the one-bet-a-day rule includes zero-bet days, and skipping IS the winning play on days like this.</div>
      </div>`;
    }}
    // FUN bucket: free-credit / fun-budget longshots (looser pool — see
    // udBuildFunLegs). Rendered as a third section below the money parlays.
    const funSel = udSelectFunParlays(udBuildFunLegs(rows));
    const funTwo = udFunColumn(funSel.two, 2);
    const funThree = udFunColumn(funSel.three, 3);
    const funBody = (funTwo || funThree)
      ? `<div class="udlab-pl-grid"><div>${{funTwo}}</div><div>${{funThree}}</div></div>`
      : `<p class="muted">No model-favored longshots on the board right now.</p>`;
    const funSection = `<div class="udlab-fun-section">
      <div class="udlab-pl-head-main">🎁 Free-credit / fun-budget longshots <span class="muted">(house money or the ~$15/wk fun budget — the 0.15 phantom cap and 0.08 floor are dropped here; ranked by expected payout, not edge. −EV on cash, but +value on free credits and accepted entertainment on the fun budget. Tap → tag it Free or Fun.)</span></div>
      ${{funBody}}
    </div>`;
    return `<div class="udlab-parlays">
      ${{diffSummary}}
      <div class="udlab-pl-head-main">UD-aware parlays <span class="muted">(<strong>2-leg only</strong> — paid 3-legs graded −49.9% lifetime, so 3-leg cash was retired 2026-07-02; longshots live in the 🎁 free-credit section below · one leg per game · games within 3 hrs · capped per pitcher · ranked by EV per $1 at UD payouts · cards that are only +EV with the 30% boost are kept and labeled <em>boost/free only</em> · 🚀 = EV with the boost · payout = base × pick multipliers, priced base 3.5 · <strong>△ unpriced</strong> = leg(s) UD hasn't priced, edge rests on the model alone — lower confidence)</span></div>
      ${{boostBanner}}
      ${{plBody}}
      ${{funSection}}
    </div>`;
  ...
  function udLabControls(dateStr) {{
    ...
    return `<div class="udlab-controls">
      ...
      <div class="udlab-savewrap">
        <button type="button" class="udlab-record-btn">📊 My record</button>
        <label class="udlab-upload">📷 Import screenshot<input type="file" class="udlab-upload-input" accept="image/*" multiple hidden></label>
  ...
  // "My record" panel — your actual betting results + the edge-band tuning
  // report (server-computed over your ledger × settled actuals; see
  // bets/bet_record.py and GET /api/bet-record). Loaded on demand by the
  // "📊 My record" button. The low-variance, no-capture replacement for the
  // old closing-line-value screenshot workflow.
  function udPct(x) {{ return x == null ? "—" : (x * 100).toFixed(0) + "%"; }}
  function udSignedPct(x) {{ return x == null ? "—" : (x >= 0 ? "+" : "") + (x * 100).toFixed(1) + "%"; }}

  function udRenderRecord(d) {{
    if (!d || !d.n_settled) {{
      return `<div class="udlab-record"><h4>📊 My betting record</h4><div class="rec-note">No settled bets yet.</div></div>`;
    }}
    const a = d.all, p = d.paid, ci = d.paid_ci, leg = d.leg, be = d.breakeven;
    let h = `<div class="udlab-record"><h4>📊 My betting record — ${{d.n_settled}} settled bets</h4>`;
    if (a) h += `<div class="rec-line"><strong>All (incl promos):</strong> ${{a.wins}}/${{a.n}} won · $${{a.staked.toFixed(0)}}→$${{a.returned.toFixed(0)}} · ROI ${{udSignedPct(a.roi)}}</div>`;
    if (p) {{
      h += `<div class="rec-line"><strong>Paid only:</strong> ${{p.wins}}/${{p.n}} won · ROI ${{udSignedPct(p.roi)}}`;
      if (ci) h += ` <span class="rec-note" style="margin:0">(95% range ${{udSignedPct(ci.lo)}} to ${{udSignedPct(ci.hi)}})</span>`;
      h += `</div>`;
    }}
    if (ci && ci.spans_zero) h += `<div class="rec-note">Not yet distinguishable from zero — more bets needed to confirm an edge.</div>`;
    if (leg && leg.rate != null) h += `<div class="rec-line" style="margin-top:8px"><strong>Per-leg hit rate:</strong> ${{leg.hits}}/${{leg.graded}} = ${{(leg.rate * 100).toFixed(1)}}% <span class="rec-note" style="margin:0">(UD needs ~${{udPct(be.leg3)}}–${{udPct(be.leg2)}} to profit)</span></div>`;
    if (d.bands && d.bands.length) {{
      h += `<table class="udlab-rec-table"><thead><tr><th>Model edge when you bet</th><th>legs</th><th>hit</th></tr></thead><tbody>`;
      for (const b of d.bands) h += `<tr><td>${{escapeHTML(b.label)}}</td><td>${{b.n}}</td><td class="${{b.above_breakeven ? "above" : "below"}}">${{(b.rate * 100).toFixed(0)}}%</td></tr>`;
      h += `</tbody></table><div class="rec-note">Bands clearing ~${{udPct(be.leg3)}} beat Underdog breakeven. As samples grow, this is what to tune the bet bar toward.</div>`;
    }}
    // Three-bucket breakdown (2026-06-15): how each lane is performing.
    if (d.buckets) {{
      const lbl = {{ focus: "Edge (focus)", boost: "Boost", default: "Legacy/untagged", free: "Free credits" }};
      const rows = ["focus", "boost", "default", "free"].filter(k => d.buckets[k]).map(k => {{
        const b = d.buckets[k];
        const cost = k === "free" ? "$0" : "$" + b.staked.toFixed(0);
        return `<tr><td>${{lbl[k]}}</td><td>${{b.n}}</td><td>${{cost}}→$${{b.returned.toFixed(0)}}</td><td class="${{(b.roi || 0) >= 0 ? "above" : "below"}}">${{udSignedPct(b.roi)}}</td></tr>`;
      }}).join("");
      if (rows) h += `<table class="udlab-rec-table" style="margin-top:8px"><thead><tr><th>Bucket</th><th>bets</th><th>cost→ret</th><th>ROI</th></tr></thead><tbody>${{rows}}</tbody></table>`;
    }}
    // Fun budget: walled off from the edge read; show spend + this-week tally.
    if (d.fun && d.fun.n) {{
      let thisWeek = 0;
      try {{
        const t = new Date(dateInChicago(0) + "T12:00:00");
        const mon = new Date(t); mon.setDate(t.getDate() - ((t.getDay() + 6) % 7));
        thisWeek = (d.fun.by_week || {{}})[mon.toISOString().slice(0, 10)] || 0;
      }} catch (e) {{}}
      h += `<div class="rec-line" style="margin-top:6px"><strong>🎁 Fun budget:</strong> $${{d.fun.spend.toFixed(0)}}→$${{d.fun.returned.toFixed(0)}} (net $${{d.fun.net.toFixed(0)}}, entertainment — not an edge claim) · <strong>this week $${{thisWeek.toFixed(0)}}/$15</strong></div>`;
    }}
    return h + `</div>`;
  }}

  async function udLoadRecord() {{
    const mount = document.getElementById("udlab-record-mount");
    if (!mount) return;
    if (mount.dataset.open === "1") {{ mount.innerHTML = ""; mount.dataset.open = "0"; return; }}
    mount.innerHTML = `<div class="udlab-record rec-note">Loading your record…</div>`;
    mount.dataset.open = "1";
    try {{
      const r = await fetch("/api/bet-record", {{ cache: "no-store" }});
      if (!r.ok) {{ mount.innerHTML = `<div class="udlab-record rec-note">Couldn't load record (${{r.status}}).</div>`; return; }}
      mount.innerHTML = udRenderRecord(await r.json());
    }} catch (e) {{
      mount.innerHTML = `<div class="udlab-record rec-note">Couldn't load record.</div>`;
    }}
  }}
  ...
  function renderUDLabTab(rows, dateStr) {{
    ...
    const intro = `<div class="udlab-intro">
        ...
        <p class="muted">You bet <strong>Underdog</strong>, but the live tabs grade edge against the <strong>sportsbook</strong> line. Each UD line is <strong>prefilled with the sportsbook line</strong>, sorted high→low like the UD board — fix the few that differ. When UD shows <strong>Higher/Lower multipliers</strong>, enter them too: that's UD pricing the pick, and we de-vig it into UD's own implied probability (the <em>Mkt P</em> column). A pick UD leaves <strong>symmetric (no multiplier)</strong> falls back to the sharp consensus at the UD line — and gets a <strong>△ caution</strong> in the parlays below: UD almost always prices the picks it disagrees on, so an unpriced "edge" rests on the model alone (early tracking has those legs underperforming). <em>Edge</em> = model's P minus the price you're actually getting; ${{calNote}}. Relievers/openers (line under ${{MIN_LINE_FOR_FOCUS}}) are greyed and skipped — the model over-projects them as full starts. Hit <em>Save all</em> to record the board. Nothing here touches the live tabs.</p>
      </div>
    ...
    return `<section class="udlab">${{intro}}
      <div id="udlab-record-mount"></div>
      <div class="table-wrap">
      <table class="udlab-table">
        <thead><tr>
          <th>Pitcher</th>
          <th title="vs = pitcher at home (opp bats top of innings), @ = away">Opp</th>
          <th title="First pitch (CT) with countdown; flips to live inning + K count once underway. Lineups typically post ~2–3h before first pitch.">Time</th>
          <th class="num">Proj</th><th class="num">Bk ln</th>
          <th>Live</th>
  ...
  async function loadUDLab(dateStr) {{
    ...
    _udImported.clear();  // imports are date-specific
  ...
  async function saveAllUDLines() {{
    ...
    for (const r of _udSlateRows) {{
      ...
      const m = udMults(r);
      // Skip rows with nothing meaningful (no line and untouched 1/1 mults).
      if (line === null && !multPriced(m.hi, m.lo)) continue;
    ...
    try {{
      ...
      _udImported.clear();  // review state resolved — drop the import highlight
      repaintUDLab();
  ...
  // Send each UD screenshot to the vision endpoint, merge extracted pricing
  // into the board as a *review* state (highlighted rows), and report
  // matched / unmatched / failures. Never auto-saves — Chad reviews + Save all.
  async function importUDScreenshots(files) {{
    if (!files || !files.length) return;
    const setStatus = (msg) => {{
      const s = document.querySelector("#udlab-panel .udlab-save-status");
    ...
    for (const file of files) {{
      let dataUrl;
      try {{ dataUrl = await readFileAsDataURL(file); }} catch (e) {{ failed++; continue; }}
      try {{
        const url = (_udSlateDate ? `/api/ud-screenshot?date=${{_udSlateDate}}` : `/api/ud-screenshot`);
        const r = await fetch(url, {{
          method: "POST",
          headers: {{ "Content-Type": "application/json" }},
        ...
        if (!r.ok) {{
          if (r.status === 503) {{
            setStatus("Screenshot import needs ANTHROPIC_API_KEY on the server — not configured yet.");
            return;
          }}
          failed++; continue;
        }}
        const d = await r.json();
        for (const row of (d.matched || [])) {{
          const pid = String(row.pitcher_id);
          _udBoard[pid] = {{ line: row.line, hi: row.higher, lo: row.lower }};
          _udImported.add(pid);
          imported++;
        }}
        for (const u of (d.unmatched || [])) unmatched.push(u.name || "?");
      }} catch (e) {{ failed++; }}
    }}
    repaintUDLab();
    let msg = `Imported ${{imported}} pitcher${{imported === 1 ? "" : "s"}} — review the highlighted rows, then Save all.`;
    if (unmatched.length) msg += ` Unmatched (enter manually): ${{unmatched.join(", ")}}.`;
    if (failed) msg += ` ${{failed}} image(s) couldn't be read.`;
    setStatus(msg);
  }}
  ...
  function wireUDLabInputs() {{
    ...
    panel.addEventListener("click", (ev) => {{
      ...
      else if (t.classList.contains("udlab-record-btn")) udLoadRecord();
    }});
  }}
  ...
  function pitcherResultRow(r) {{
    ...
    // Classify with the calibrated primary edge (v2bc since 2026-06-22 →
    // v2 → raw for older rows) so the "Our Pick" tag matches what Today's
    // Picks would actually have surfaced (graded-surface-parity rule).
    const bcEdge = f(slateOrLive(r, "slate_cal_edge_v2bc", "cal_edge_v2bc"));
    const calEdge = f(slateOrLive(r, "slate_cal_edge_v2", "cal_edge_v2"));
    const rawEdge = f(slateOrLive(r, "slate_edge", "edge"));
    const edge = bcEdge !== null ? bcEdge : (calEdge !== null ? calEdge : rawEdge);
    const cls = classify(edge);
    const dir = edge === null || edge === 0 ? "" : (edge > 0 ? "over" : "under");
  ...
  async function fetchTrackRecord(maxDays = 14) {{
    ...
    for (const {{ d, text }} of results) {{
      ...
      for (const r of rows) {{
        ...
        // Prefer calibrated v2bc (primary since 2026-06-22), slate-time when
        // present — same signal the live dashboard's pickEdge() uses for
        // classification (graded-surface-parity rule). Falls back to
        // cal_edge_v2 for rows predating the v2bc shadow (pre-2026-06-09),
        // then raw slate_edge/edge for pre-Platt rows (pre-2026-05-11).
        const bcEdge = f(slateOrLive(r, "slate_cal_edge_v2bc", "cal_edge_v2bc"));
        const calEdge = f(slateOrLive(r, "slate_cal_edge_v2", "cal_edge_v2"));
        const rawEdge = f(slateOrLive(r, "slate_edge", "edge"));
        const edge = bcEdge !== null ? bcEdge : (calEdge !== null ? calEdge : rawEdge);
      ...
      <div class="bets-form-bottom">
        ...
        <div class="bets-field">
          ...
          <select id="bf-reason" style="background: var(--bg); color: var(--text); border: 1px solid var(--border); border-radius: 5px; padding: 7px 9px; font-family: inherit; font-size: 13px;">
            ...
            <option value="fun">fun (lottery budget)</option>
  ...
  function clearForm() {{
    _pendingBetContext = null;  // abandoned handoff must not tag a later manual bet
  ...
  function wireBetsHandlers(panel) {{
    ...
    if (saveBtn) {{
      saveBtn.addEventListener("click", async () => {{
        ...
        // Provenance tagging (2026-06-04): which suggester produced this
        // ticket + the card as displayed at tap time. Stamped on ADD only —
        // edits must never overwrite the original placement context.
        if (!editingBetId) {{
          data.source = "manual";
          if (_pendingBetContext) {{
            const sig = _legsSig(goodLegs);
            const tapped = _pendingBetContext.legsSig.split("|");
            const overlap = sig.split("|").filter(s => tapped.includes(s)).length;
            if (sig === _pendingBetContext.legsSig) {{
              data.source = _pendingBetContext.source;
              data.suggested_card = _pendingBetContext.card;
            }} else if (overlap > 0) {{
              // Tapped a card, then tweaked it — keep the source but mark
              // the snapshot edited so analysis can separate exact-card
              // bets from modified ones.
              data.source = _pendingBetContext.source;
              data.suggested_card = {{ ..._pendingBetContext.card, edited: true }};
            }}
            // Zero overlap: the tapped card was fully replaced by hand —
            // honest tag is "manual", and the stale card snapshot is dropped.
          }}
        }}
        try {{
          if (editingBetId) {{
  ...
  function renderPitcherTab(target) {{
    ...
    if (settled.rows.length) {{
      ...
      // surfaces — i.e. isBettableFocus: calibrated primary edge (v2bc since
      // 2026-06-22) in the focus band AND line >= MIN_LINE_FOR_FOCUS (gates
      // out openers/relievers). Falls back to cal_edge_v2 for rows predating
      // the v2bc shadow (pre-2026-06-09), then raw edge for pre-Platt
      // historical rows (pre-2026-05-11).
      const dayPicks = [];
  ...
  // Provenance context for the bet being composed (2026-06-04). Set when a
  // suggested parlay card is tapped, consumed by the save handler (which
  // stamps source + suggested_card onto the POST), cleared on save/cancel.
  // Manual entries (no tap) save as source "manual".
  let _pendingBetContext = null;

  function _legsSig(legs) {{
    // Order-independent pid:side signature, used to detect whether the
    // form's legs still match the tapped card at save time.
    return legs.map(l => `${{String(l.pitcher_id)}}:${{l.ou}}`).sort().join("|");
  }}

  async function handleAddParlayToBets(legsJSON, cardJSON) {{
    let legs;
    try {{ legs = JSON.parse(legsJSON); }} catch (e) {{ return; }}
    if (!Array.isArray(legs) || legs.length < 2) return;
    let card = null;
    try {{ card = cardJSON ? JSON.parse(cardJSON) : null; }} catch (e) {{ card = null; }}
    _pendingBetContext = card && card.source
      ? {{ source: card.source, card, legsSig: _legsSig(legs) }}
      : null;
    showTab("bets");
    ...
    recomputeCombined();
    // Auto-tag the three-bucket lane from the tapped card so the betting
    // record self-populates (Chad can still override the dropdown). A FUN
    // longshot card → "fun"; the daily boost target → "boost"; a clean
    // cash-eligible card → "focus"; otherwise (boost/free-only) → "boost".
    // free_entry is set via its own checkbox (server coerces that reason).
    const reasonEl = document.getElementById("bf-reason");
    if (reasonEl && card) {{
      reasonEl.value = card.kind === "fun" ? "fun"
        : card.boost_target ? "boost"
        : card.cash_eligible ? "focus"
        : "boost";
    }}
    // One-bet-per-day policy (2026-07-02): prefill the $5 stake so a tapped
    // card is save-ready, and pre-type the boost note on the boost-target
    // card. Both stay editable — free-entry stakes vary (the checkbox path
    // is unaffected), and the boost note should be cleared if the boost is
    // already spent and the card is being played plain.
    const stakeEl = document.getElementById("bf-stake");
    if (stakeEl && card) stakeEl.value = String(BOOST_STAKE);
    const boostEl = document.getElementById("bf-boost");
    if (boostEl && card) boostEl.value = card.boost_target ? "30%" : "";
    const form = document.getElementById("bets-form");
  ...
  document.addEventListener("DOMContentLoaded", () => {{
    ...
    document.addEventListener("click", (e) => {{
      const card = e.target.closest(".parlay-card[data-legs], .udlab-pl-card[data-legs], .udlab-boost-banner[data-legs]");
      if (!card || !isBets()) return;
      handleAddParlayToBets(card.dataset.legs || "[]", card.dataset.card || "");
    }});
