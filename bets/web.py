...
(() => {{
  ...
  function renderParlayCard(p) {{
    ...
    const cardMeta = {{
      source: "pitchers",
      ev: r4(p.ev), prob: r4(p.combinedHit), edge: r4(p.combinedEdge),
  ...
  function udParlayCard(x) {{
    ...
    const cardMeta = {{
      source: "ud_lab",
      ev: r4(x.ev), prob: r4(x.prob), pay_mult: r4(x.payMult),
    ...
    const cautionChip = caution
      ...
      : "";
    return `<div class="udlab-pl-card ${{lineupCls}}"${{dataAttr}} title="${{escapeHTML(lineupTip)}}">
      ${{overlapBadge}}
      <div class="udlab-pl-head">${{cautionChip}}${{x.confirmed ? "" : "~"}}${{x.payMult.toFixed(2)}}× · win ${{pct1(x.prob)}} · <span class="${{x.ev > 0 ? "pos" : "neg"}}">EV ${{(x.ev >= 0 ? "+" : "")}}${{x.ev.toFixed(3)}}</span></div>
      ${{legHtml}}
    </div>`;
  }}
  ...
  function renderUDParlayCompare(rows) {{
    ...
    return `<div class="udlab-parlays">
      ${{diffSummary}}
      <div class="udlab-pl-head-main">UD-aware parlays <span class="muted">(Positive-EV combos · one leg per game · games within 3 hrs · capped per pitcher · ranked by EV per $1 · 3-legs marked ⚠ if they overlap the top 2-leg · payout = base × pick multipliers, boosted base 3.5 (2-leg) / 6.5 (3-leg) · <strong>△ unpriced</strong> = leg(s) UD hasn't priced, edge rests on the model alone — lower confidence)</span></div>
      ${{plBody}}
    </div>`;
  }}
