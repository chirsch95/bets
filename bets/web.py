...
(() => {{
  ...
  function renderHeroWhy(r) {{
    ...
    const framing = f(r.framing_factor);
    const weather = f(r.weather_factor);
    ...
    if (framing !== null && Math.abs(framing - 1.00) > 0.002) {{
      const t = tilt(framing, 1.00, 0.005);
      const catcher = (r.catcher_name || "").split(",")[0] || "";
      const lbl = catcher ? `Catcher framing (${{catcher}})` : "Catcher framing";
      items.push(`<li><span class="why-label">${{lbl}}</span><span class="why-val">${{framing.toFixed(3)}}</span><span class="why-tilt ${{t.cls}}">${{t.sym}}</span></li>`);
    }}
    if (weather !== null && Math.abs(weather - 1.00) > 0.002) {{
      const t = tilt(weather, 1.00, 0.003);
      const cond = r.weather_condition || "";
      const temp = r.weather_temp ? `${{r.weather_temp}}°` : "";
      const wind = r.weather_wind || "";
      const detail = [cond, temp, wind].filter(Boolean).join(" · ");
      const lbl = detail ? `Weather (${{detail}})` : "Weather";
      items.push(`<li><span class="why-label">${{lbl}}</span><span class="why-val">${{weather.toFixed(3)}}</span><span class="why-tilt ${{t.cls}}">${{t.sym}}</span></li>`);
    }}
