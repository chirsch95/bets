...
(() => {{
  ...
  function udSelectParlays(legs) {{
    ...
    // Boost allocation: the daily 30% boost belongs on the single card with
    // the highest boosted EV across BOTH sections. Boost scales the payout,
    // so this is also the highest expected-payout card, period — there is
    // never a reason to split hairs between "best for cash" and "best for
    // the boost"; they're the same card.
    const all = [...two, ...three];
    if (all.length) {{
      const best = all.reduce((a, b) => (b.evBoost > a.evBoost ? b : a));
  ...
  function renderUDParlayCompare(rows) {{
    ...
    const three = udParlayColumn(sel.three, 3);
    ...
    const allCards = [...sel.two, ...sel.three];
    let boostBanner = "";
    if (allCards.length) {{
    ...
    return `<div class="udlab-parlays">
      ${{diffSummary}}
      <div class="udlab-pl-head-main">UD-aware parlays <span class="muted">(one leg per game · games within 3 hrs · capped per pitcher · ranked by EV per $1 at UD payouts · cards that are only +EV with the 30% boost are kept and labeled <em>boost/free only</em> · 🚀 = EV with the boost · 3-legs marked ⚠ if they overlap the top 2-leg · paid 3-legs are −48% lifetime, prefer free credits there · payout = base × pick multipliers, priced base 3.5 (2-leg) / 6.5 (3-leg) · <strong>△ unpriced</strong> = leg(s) UD hasn't priced, edge rests on the model alone — lower confidence)</span></div>
      ${{boostBanner}}
      ${{plBody}}
      ${{funSection}}
    </div>`;
