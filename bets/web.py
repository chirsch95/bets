...
(() => {{
  ...
  document.addEventListener("mousemove", (e) => {{
    ...
    if (readout) {{
      ...
      readout.innerHTML = `<strong>${{escapeHTML(date)}}</strong> · `
        + `<span class="cum ${{cumCls}}">${{fmtSparkVal(cumV, isMoney)}}</span>`
        + ` · ${{dailyStr}}`;
      // Position in CSS px relative to the wrap. The SVG fills the wrap
      // (width 100%), so px-x scales linearly with viewBox-x.
