...
**Status:** UD Lab tab shipped and substantially built out (`39a3a81`→`8c9ebdf`, live on Air). Captures UD's **line AND Higher/Lower payout multipliers**, de-vigs the multipliers into UD's *own* implied probability, and grades the model against the price actually paid — with the **confirmed UD payout formula** `base × ∏(side multipliers)`, boosted base = standard + 0.5 (2-leg 3.5, 3-leg 6.5). Symmetric (unpriced) picks fall back to the sharp-market reference (soft-line ★ targets). Capture is now low-friction: **📷 screenshot import** (Claude vision → extract line+multipliers → match to slate → review → save; verified 18/18 on real screenshots). Reliever/opener gate (sub-3.0 line) excludes phantom edges. #1 is now *measurable end-to-end*; the actual answer comes after captured pricing accumulates + the first 5/29 read.
...
The UD Lab tab was built this session to attack #1/#2. Chad also surfaced a key piece Claude hadn't asked about: UD's **per-pick Higher/Lower payout multipliers** (e.g. 1.04 / 0.87), shown on some pitchers. These are UD pricing the pick — de-vigging them yields UD's own implied probability, the most direct read of the market actually bet. Capturing them turned #1 from "compare to a proxy (sportsbook)" into "compare to UD's literal price." The UD payout formula was reverse-engineered and confirmed from real entries: `base × ∏(chosen-side multipliers)`, boosted base = standard + 0.5 (2-leg 3.5, 3-leg 6.5). Two more capabilities landed the same session: a **reliever/opener gate** (sub-3.0-line arms excluded — Chad noticed they skewed the picks), a **"⏰ place early" flag** on soft-line-driven parlays (time-sensitive edges that vanish when UD wakes up), and **📷 screenshot import** — upload a UD board screenshot, Claude vision extracts line + multipliers and matches to the slate (verified 18/18 on real screenshots), dropped into a review state before save. This required adding `ANTHROPIC_API_KEY` to the Air's `.env` (`bets/ud_vision.py`). Next: the first post-settle read on 2026-05-29 (see NEXT_SESSION.md).
